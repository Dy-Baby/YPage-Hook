...
static LONG NTAPI ExceptionHandler(EXCEPTION_POINTERS* ExceptionInfo) {
	...
	if (ExceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_ACCESS_VIOLATION) {
		...
		// 同步一下页面属性
		it_base->second.protect = oldProtect;
		...
		if (it_addr->second.m_hookAddr == ExceptionInfo->ExceptionRecord->ExceptionAddress) {
	...
	else if (ExceptionInfo->ExceptionRecord->ExceptionCode == EXCEPTION_SINGLE_STEP)
	{
		...
		if (pContext->Dr6 & 0xf) {
			...
			LPVOID pageBase = it->second.pageBase;
...
void PageHook::install(LPVOID hookAddr, HookCallBack callback) {
	...
	auto it = gs_pageHook_addr.find(hookAddr);
	if (it != gs_pageHook_addr.end()) {
	...
	if (it_base == gs_pageHook_base.end()) {
		...
		if (!VirtualProtect(pageBase, 0x1000, PAGE_READWRITE, &pageRecord.protect)) {
			uninstall();
